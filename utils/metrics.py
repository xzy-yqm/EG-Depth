...
@make_nograd_func
@compute_metric_for_each_image
def iMAE_metric(D_est, D_gt, mask, conversion_rate):
    #D_est, D_gt = D_est[mask]/589.09, D_gt[mask]/589.09
    #D_est, D_gt = D_est[mask]/518.4, D_gt[mask]/518.4
    D_est, D_gt = D_est[mask]/conversion_rate, D_gt[mask]/conversion_rate
    E = torch.abs(D_gt - D_est)
    return torch.mean(E.float())

@make_nograd_func
@compute_metric_for_each_image
def iRMSE_metric(D_est, D_gt, mask, conversion_rate):
    #D_est, D_gt = D_est[mask]/589.09, D_gt[mask]/589.09
    #D_est, D_gt = D_est[mask]/518.4, D_gt[mask]/518.4
    D_est, D_gt = D_est[mask]/conversion_rate, D_gt[mask]/conversion_rate
...
@make_nograd_func
@compute_metric_for_each_image
def MAE_metric(D_est, D_gt, mask, conversion_rate):
    #D_est, D_gt = 589.09/D_est[mask], 589.09/D_gt[mask]
    #D_est, D_gt = 518.4/D_est[mask], 518.4/D_gt[mask]
    D_est, D_gt = conversion_rate/D_est[mask], conversion_rate/D_gt[mask]
    E = torch.abs(D_gt - D_est)
    return torch.mean(E.float())

@make_nograd_func
@compute_metric_for_each_image
def RMSE_metric(D_est, D_gt, mask, conversion_rate):
    #D_est, D_gt = 589.09/D_est[mask], 589.09/D_gt[mask]
    #D_est, D_gt = 518.4/D_est[mask], 518.4/D_gt[mask]
    D_est, D_gt = conversion_rate/D_est[mask], conversion_rate/D_gt[mask]
    E = F.mse_loss(D_est, D_gt, size_average=True)
